...
!- this for the namelist wsm.inp
   namelist /run/ prefix, time,mcphys_type,mynum,sedim, &
                 cold_phase, warm_phase, cond_evap
...
!- to save initial values
   integer :: nvar_out
   real, allocatable, dimension(:,:,:,:)  :: x3d
   integer, parameter :: nvar = 5
   integer, parameter :: ithe=1, irtp=2,irv=3, ircp=4, irrp=5 
   character(len=4), parameter, dimension(nvar) :: var = &
   (/     &
    'ithe' & ! 1
   ,'irtp' & ! 2
   ,'irv ' & ! 3
...
!- read namelist  
   ...
   !=======================================================================
    ...
    open(newunit = l_unit, file=trim(fileout)//".ctl", action='write', status='replace')
       ...
       !--- input data 
       write(l_unit,*) var(ithe) ,nlev,'99 ','K'
       write(l_unit,*) var(irtp) ,nlev,'99 ','K'
       write(l_unit,*) var(irv ) ,nlev,'99 ','K'
       write(l_unit,*) var(ircp) ,nlev,'99 ','K'
       write(l_unit,*) var(irrp) ,nlev,'99 ','K'
