program wsm_test
   ...
   use module_mp_wsm3, only: wsm3init, wsm3,sedim, cold_phase&
                           , warm_phase, cond_evap, sedim_after &
                           , cond_evap_before, nvar,invar
   ...
   integer :: l_unit = 4, i,j,k,kr,nsteps
   character(len=5) :: ctime,cmynum,cit
...
!- this for the namelist wsm.inp
   namelist /run/ prefix, time,mcphys_type,mynum,sedim,cold_phase &
                , warm_phase, cond_evap,nsteps,sedim_after        &
                , cond_evap_before
...
!- to save initial values
   integer :: nvar_out, lvar , it
   real, allocatable, dimension(:,:,:,:)  :: x3d,i3d,i3dwsm
  !integer, parameter :: nvar = 5,invar=5
   integer, parameter :: ithe=1, irtp=2,irv=3, ircp=4, irrp=5 
   character(len=4), parameter, dimension(nvar) :: cvar = &
   (/     &
    'ithe' & ! 1
   ,'irtp' & ! 2
   ,'irv ' & ! 3
   ...
   ,'irrp' & ! 5
   /)
  character(len=4), parameter, dimension(invar) :: cinvar = &
   (/     &
    'iv1' & ! 1
   ,'iv2' & ! 2
   ,'iv3' & ! 3
...
!- read namelist  
   ...
   open(newunit = l_unit,file =trim(filein),ACCESS = "stream",action="read", status="old")
      ...
      if(first_read) then
         ...
         allocate(temp (m1,m2,m3))   ;temp  = 0.0
         ...
         allocate(x3d       ( m1,m2,m3,nvar )) ;x3d = 0.0 
         !-- variables to save internal arrays of WSM*
         allocate(i3d       ( m1,m2,m3,invar)) ;i3d = 0.0
         allocate(i3dwsm    ( ims:ime, kms:kme, jms:jme, invar )) ; i3dwsm = 0.0
      endif
   ...
   !------------------------------ saving input data --------------------------------
    x3d(1:m1,1:m2,1:m3, ithe) = theta  (1:m1,1:m2,1:m3)
    x3d(1:m1,1:m2,1:m3, irtp) = rtp    (1:m1,1:m2,1:m3)
    x3d(1:m1,1:m2,1:m3, irv ) = rv     (1:m1,1:m2,1:m3)
    x3d(1:m1,1:m2,1:m3, ircp) = rcp    (1:m1,1:m2,1:m3)
    x3d(1:m1,1:m2,1:m3, irrp) = rrp    (1:m1,1:m2,1:m3)
   cit = '00000'

do it = 1,nsteps ! loop in time 
   write(cit,fmt="(I5.5)") it
   ...
   do j = ja,jz
    ...
    !- column quantities
      ...
      do k = 1,kme-1
        ...
        !print*,'dz',k,kr,zt(k),1./dzt(k), dz8w   (1,k,1)
      enddo
      ...
      IF(mcphys_type == 30)                   &   
           ...
           CALL wsm3(                         &
                   ...
                   !-- srf additional array for output of internal arrays
                   i3dwsm &
                   )
      ...
      !- updated variables after microphysics processes (from WSM to BRAMS)
        ...
        !- internal variables saved for output
         do lvar = 1, invar
             do k=1,kme-1
                kr=k+1
                i3d(kr,i,j,lvar) =  i3dwsm (1,k,1,lvar)
             enddo
             i3d(1,i,j,lvar) = i3d(2,i,j,lvar)  
         enddo
        ...
        !- column quantities
         ...
         !-- air temperature 
         temp(1:m1,i,j) = theta(1:m1,i,j)*(pp(1:m1,i,j)+pi0(1:m1,i,j))*cpi - 273.15
   
   enddo; enddo ! loop i,j
   !
   !
   if(it /= 1 .and. it /= nsteps) cycle 
   !
   !=======================================================================
    ...
    fileout=trim(prefix)//"_WSM"//trim(cmcphys_type)//"_dataOut-"//ctime//"-"//trim(cmynum)//"-"//trim(cit)
    ...
    open(newunit = l_unit, file=trim(fileout)//".gra", &
        ...
        nvar_out = 0
        !- input data 
        do nz=nz1,nz2;  write(l_unit,rec=irec) x3d (nz,1:m2,1:m3,ithe)      ; irec=irec+1 ;enddo; nvar_out = nvar_out + 1
        do nz=nz1,nz2;  write(l_unit,rec=irec) x3d (nz,1:m2,1:m3,irtp)*1000.; irec=irec+1 ;enddo; nvar_out = nvar_out + 1
        do nz=nz1,nz2;  write(l_unit,rec=irec) x3d (nz,1:m2,1:m3,irv )*1000.; irec=irec+1 ;enddo; nvar_out = nvar_out + 1
        do nz=nz1,nz2;  write(l_unit,rec=irec) x3d (nz,1:m2,1:m3,ircp)*1000.; irec=irec+1 ;enddo; nvar_out = nvar_out + 1
        do nz=nz1,nz2;  write(l_unit,rec=irec) x3d (nz,1:m2,1:m3,irrp)*1000.; irec=irec+1 ;enddo; nvar_out = nvar_out + 1

        !- 3d updated fields
        do nz=nz1,nz2;  write(l_unit,rec=irec) wp         (nz,1:m2,1:m3); irec=irec+1 ;enddo; nvar_out = nvar_out + 1
        do nz=nz1,nz2;  write(l_unit,rec=irec) temp       (nz,1:m2,1:m3); irec=irec+1 ;enddo; nvar_out = nvar_out + 1
        ...
        !- 2d 
        write(l_unit,rec=irec) 3600*accpr(1:m2,1:m3); irec=irec+1 ; nvar_out = nvar_out + 1 ! units mm/h
        write(l_unit,rec=irec) 3600*pcprr(1:m2,1:m3); irec=irec+1 ; nvar_out = nvar_out + 1 ! units mm/h

        !- additional output data (from internal WSM)
        do lvar=1,invar
          do nz=nz1,nz2;  write(l_unit,rec=irec) i3d(nz,1:m2,1:m3,lvar); irec=irec+1 ;enddo
          if(lvar==1) print*,"max",maxval(i3d(:,:,:,lvar)),minval(i3d(:,:,:,lvar))
          nvar_out = nvar_out + 1
        enddo
    ...
    open(newunit = l_unit, file=trim(fileout)//".ctl", action='write', status='replace')
       ...
       !--- input data 
       write(l_unit,*) cvar(ithe) ,nlev,'99 ','K'
       write(l_unit,*) cvar(irtp) ,nlev,'99 ','K'
       write(l_unit,*) cvar(irv ) ,nlev,'99 ','K'
       write(l_unit,*) cvar(ircp) ,nlev,'99 ','K'
       write(l_unit,*) cvar(irrp) ,nlev,'99 ','K'
       !--- updated data
       write(l_unit,*) 'wp'    ,nlev,'99 ','K'
       write(l_unit,*) 'temp'  ,nlev,'99 ','C'
       ...
       do lvar=1,invar
        write(l_unit,*) cinvar (lvar) ,nlev,'99 ','K'
       enddo
