...
LOGICAL, PARAMETER, PRIVATE:: GRAUPEL = .FALSE.
LOGICAL, PARAMETER, PRIVATE:: CUPAR = .false.
real, parameter ::   g   = 9.80  , f=2., C_d=0.506, gam=0.5, beta=1.875
...
!-SRF moving optional arrays for the end of the argument list
      ...
      REAL, DIMENSION(its:ite,kts:kte), INTENT(IN):: hgt

!..OPTIONAL variables that control application of aerosol-aware scheme

      REAL, DIMENSION(its:ite,kts:kte), OPTIONAL, INTENT(INOUT) :: nwfa, nifa
      REAL, DIMENSION(its:ite), OPTIONAL, INTENT(INOUT) :: nwfa2d
      ...
      write(mp_debug,*) ' DEBUG  checking column of hgt ', its
      CALL wrf_debug(250, mp_debug)
      do k = kts, kte
         write(mp_debug,*) ' DEBUGT  k, hgt = ', k, hgt(its,k)
         CALL wrf_debug(250, mp_debug)
      enddo
...
!.. fill in just a basic vertical profile, somewhat boundary-layer following.
      ...
      if (max_test .lt. eps) then
         ...
         do i = its, ite
            ...
            print*,"initial CCN aerosols=",i,1,nwfa(i,1)
            do k = 2, kte
               nwfa(i,k) = naCCN1+naCCN0*exp(-((hgt(i,k)-hgt(i,1))/1000.)*niCCN3)
               !print*,"initial CCN aerosols=",i,k,nwfa(i,k),hgt(i,k)
            enddo
         enddo
      else
         has_CCN    = .TRUE.
         ...
         CALL wrf_debug(100, mp_debug)
         write(mp_debug,*) '   column sum at point (i) = ', its, SUM(nwfa(its,:))
         CALL wrf_debug(100, mp_debug)
      endif


      max_test = MAXVAL ( nifa(its:ite-1,:) )
      if (max_test .lt. eps) then
         ...
         do i = its, ite
            ...
            print*,"initial IN  aerosols=",i,1,nifa(i,1)
            do k = 2, kte
               nifa(i,k) = naIN1+naIN0*exp(-((hgt(i,k)-hgt(i,1))/1000.)*niIN3)
               !print*,"initial IN  aerosols=",i,k,nifa(i,k)
            enddo
         enddo
      else
         has_IN     = .TRUE.
         ...
         CALL wrf_debug(100, mp_debug)
         write(mp_debug,*) '   column sum at point (i) = ', its, SUM(nifa(its,:))
         CALL wrf_debug(100, mp_debug)
      endif
...
!.. for a grid with 20km spacing and scale accordingly for other spacings.
      ...
      if (is_start) then
         ...
         CALL wrf_debug(100, mp_debug)
          do i = its, ite
            nwfa2d(i) = 10.0**(LOG10(nwfa(i,kts)*1.E-6)-3.69897)
...
!+---+-----------------------------------------------------------------+
      SUBROUTINE mp_gt_driver(mynumx,qv, qc, qr, qi, qs, qg, ni, nr,           &
                              temp,  p, w, dz, dt_in, itimestep,      &
                              RAINNC, RAINNCV, &
                              SNOWNC, SNOWNCV, &
...
!..Subroutine arguments
      !~ INTEGER, INTENT(IN):: ids,ide, jds,jde, kds,kde, &
                            ...
                            !~ its,ite, jts,jte, kts,kte   ,mynumx

      INTEGER, INTENT(IN):: its,ite, kts,kte,ktf   ,mynumx

      REAL, DIMENSION(its:ite, kts:kte), INTENT(INOUT):: &
                          qv, qc, qr, qi, qs, qg, ni, nr, temp
      REAL, DIMENSION(its:ite, kts:kte), OPTIONAL, INTENT(INOUT):: &
                          nc, nwfa, nifa
      REAL, DIMENSION(its:ite), OPTIONAL, INTENT(IN):: nwfa2d
      REAL, DIMENSION(its:ite, kts:kte), INTENT(INOUT):: &
                          re_cloud, re_ice, re_snow
      ...
      REAL, DIMENSION(its:ite), INTENT(INOUT):: &
                          RAINNC, RAINNCV, SR
      REAL, DIMENSION(its:ite), OPTIONAL, INTENT(INOUT)::      &
...
!- environmental state and other variables for cumulus parameterization 
      integer, dimension (its:ite),intent (in)    :: ierr,kbcon,ktop,klcl,kpbli
      REAL, DIMENSION(its:ite, kts:kte), INTENT(IN):: &
                     temp_env,                        &
           qv_env  ,                        &
                     zuo,up_massentr,up_massdetr,entr_rate_2d,cd,dz_cup 
      REAL, DIMENSION(its:ite, kts:kte), OPTIONAL, INTENT(IN):: &
          nwfa_env                                 &
                     ,nifa_env                       
!-
!..Local variables 
...
!-srf new var added
      ...
      REAL, DIMENSION(kts:kte):: qv1d_env,t1d_env,en1d,cd1d &
                                ,zuo1d ,up_massentr1d,up_massdetr1d &
        ,nwfa1d_env, nifa1d_env,dz1d_cup    
      REAL, DIMENSION(kts:kte):: w1dtmp
!-srf

REAL, DIMENSION(its:ite,kts:kte):: lixo
...
!
      ...
      if(show_neg_scalar) then
         qc_max  = 0.; qr_max  = 0.; qs_max  = 0.; qi_max  = 0.; qg_max  = 0.; ni_max  = 0.; nr_max  = 0.
         imax_qc = 0 ; imax_qr = 0 ; imax_qi = 0 ; imax_qs = 0 ; imax_qg = 0 ; imax_ni = 0 ; imax_nr = 0
         jmax_qc = 0 ; jmax_qr = 0 ; jmax_qi = 0 ; jmax_qs = 0 ; jmax_qg = 0 ; jmax_ni = 0 ; jmax_nr = 0
         kmax_qc = 0 ; kmax_qr = 0 ; kmax_qi = 0 ; kmax_qs = 0 ; kmax_qg = 0 ; kmax_ni = 0 ; kmax_nr = 0
         do i = 1, 256
            mp_debug(i:i) = char(0)
         enddo
      endif
      ...
      i_loop:  do i = i_start, i_end
         ...
         ENDIF
         SR(i) = 0.
!--loop in istart,iend => all arrays will (i) or (i,k)
!-- use ierr(i)
!-- bring temp of cloud and T of env, the same for qv and W.
      ...
      dz1d_cup(k)=dz_cup(i,k)
         ...
         if (is_aerosol_aware) then
            do k = kts, ktf
               ...
               nwfa1d(k) = nwfa(i,k)
               nifa1d(k) = nifa(i,k)
!-- conv parameterization section
         nwfa1d_env(k)= nwfa_env(i,k)
         nifa1d_env(k)= nifa_env(i,k)     
            enddo
            nwfa1 = nwfa2d(i)
         else
            do k = kts, ktf
               ...
               nc1d(k) = Nt_c/rho(k)
               nwfa1d(k) = 11.1E6/rho(k)   !air parcel?
               nifa1d(k) = naIN1*0.01/rho(k)
!-- conv parameterization section
!---think about this
         nwfa1d_env(k)= 11.1E6/rho(k) !environmental air?
         nifa1d_env(k)= naIN1*0.01/rho(k)     
...
!-- conv parameterization section
        do k = kts, ktf
      qv1d_env(k) = qv_env  (i,k)
      t1d_env (k) = temp_env(i,k)
      en1d    (k) = entr_rate_2d(i,k)
      cd1d    (k) = cd(i,k)
            w1dtmp  (k) = w(i,k)
      zuo1d   (k) = zuo(i,k)
      up_massentr1d(k)=up_massentr(i,k)
      up_massdetr1d(k)=up_massdetr(i,k)
!            print*,"ENV2=",k,qv1d(k),qv1d_env(k),t1d(k),t1d_env (k)
            print*,"ENV2=",k,zuo1d   (k),up_massentr1d(k),up_massdetr1d(k)
  enddo
...
go to 444
 open(10, FILE="cloud2.txt",STATUS='old',FORM='formatted')
        DO k=kts,kte
          ...
          read(10,101) kk, t1d(k) , temp(i,k), lixo(i,k),&
            p1d(k) , & 
            w1d(k) , &
            dz1d(k) ,&
            qv1d(k) ,&
...
444 continue
!------------------
if(maxval(w1d(:))>3.) then
write(mynum+20,*)"1 =========================================="
        DO k=kts,ktf
          write(mynum+20,101) k, t1d(k) , temp(i,k), lixo(i,k),&
            p1d(k) , & 
            w1d(k) , &
            dz1d(k) ,&
            qv1d(k) ,&
...
!srf===========
         ...
         call mp_thompson(qv1d, qc1d, qi1d, qr1d, qs1d, qg1d, ni1d,     &
                      ...
                      kts, ktf, dt, i, j &!)
!srf--for conv parameterization                        
                     ,kbcon(i),ktop(i)                                  & 
                     ,qv1d_env,t1d_env,nwfa1d_env,nifa1d_env            &
         ,en1d,cd1d,dz1d_cup  &
         ,zuo1d ,up_massentr1d,up_massdetr1d,w1dtmp)          
!srf-----------------------------------------------------------------------
...
!.. number tendency (number per kg per second).
         if (is_aerosol_aware) then
            nwfa1d(kts) = nwfa1d(kts) + nwfa2d(i)*dt_in
            do k = kts, ktf
               nc(i,k) = nc1d(k)
               nwfa(i,k) = nwfa1d(k)
               nifa(i,k) = nifa1d(k)
...
!srf===========
         ...
         IF (has_reqc.ne.0 .and. has_reqi.ne.0 .and. has_reqs.ne.0) THEN
          ...
          call calc_effectRad (t1d, p1d, qv1d, qc1d, nc1d, qi1d, ni1d, qs1d,  &
                      re_qc1d, re_qi1d, re_qs1d, kts, ktf)
          do k = kts, ktf
             re_cloud(i,k) = MAX( 2.51E-6, MIN(re_qc1d(k), 50.E-6))
             re_ice(i,k)   = MAX( 5.01E-6, MIN(re_qi1d(k), 125.E-6))
             re_snow(i,k)  = MAX(10.01E-6, MIN(re_qs1d(k), 999.E-6))
          enddo
         ENDIF
...
!..Sub arguments
      ...
      REAL, DIMENSION(kts:kte), INTENT(INOUT):: &
                          qv1d, qc1d, qi1d, qr1d, qs1d, qg1d, ni1d, &
                          nr1d, nc1d, nwfa1d, nifa1d, t1d, w1d
      REAL, DIMENSION(kts:kte), INTENT(IN):: p1d, dzq
...
!..Local variables
      ...
      INTEGER:: i, k, k2, n, nn, nstep, k_0, kbot, IT, iexfrq,kk
...
!-srf-- for conv paramerization      
      INTEGER, INTENT(IN):: kbcon,ktop
      REAL, DIMENSION(kts:kte), INTENT(IN):: qv1d_env,t1d_env    &
                                ,en1d,cd1d,nwfa1d_env,nifa1d_env &
                                ,zuo1d ,up_massentr1d,up_massdetr1d,dz1d_cup
      REAL, DIMENSION(kts:kte):: w1dtmp
      REAL Tv,TVe,BU,dw1,dw2,kx,KE_up,qtot,denom,XZ,XD,XE
      REAL, DIMENSION(kts:kte) :: &
                          b_qv1d, b_qc1d, b_qi1d, b_qr1d, b_qs1d, b_qg1d, b_ni1d, &
                          b_nr1d, b_nc1d, b_nwfa1d, b_nifa1d, b_t1d
     !--tmp
      integer   :: nsteps
      LOGICAL :: ENTR=.true.
      real :: aux1,aux2,aux3
...
!     if (ii.eq.901 .and. jj.eq.379) debug_flag = .true.
      if(debug_flag) then
        write(mp_debug, *) 'DEBUG INFO, mp_thompson at (i) ', ii, ', ', jj
...
!..Bug fix (2016Jun15), prevent use of uninitialized value(s) of snow moments.
      do k = kts, kte
         ...
         smof(k) = 0.
      enddo
!-srf-- for conv parameterization   :  backup state
      do k = kts, kte
            b_qv1d(k)   =   qv1d(k)  
            b_qc1d(k)   =   qc1d(k)  
            b_qi1d(k)   =   qi1d(k)  
...
!-srf-part_0
!-srf-- for conv parameterization     
!- 1st : transport upwards and mix laterally with the environment
 nsteps=0 
if(k>kbcon-1 + nsteps) then
          !-- mixing 
      XZ=zuo1d(k-1)
      XD=0.5*up_massdetr1d(k-1)
      XE=up_massentr1d(k-1)
      denom =  (XZ-XD+XE)
!--tmp
IF(.not. ENTR) denom=-9999.  !;print*,"denom=",k,denom,XZ,XD,XE
!--tmp
            if(denom > 0.) then
            !-- mixing 
        !qv1d(k)  = (qv1d(k-1)*zuo1d(k-1)-0.5*up_massdetr1d(k-1)* qv1d    (k-1) +   &
              !                                     up_massentr1d(k-1)* qv1d_env(k-1)     )/ denom 
               qv1d(k)  = (qv1d(k-1)*XZ - XD* qv1d(k-1) + XE* qv1d_env(k-1) )/ denom 
                t1d(k)  = ( t1d(k-1)*XZ - XD* t1d (k-1) + XE* t1d_env (k-1) )/ denom 

               qc1d(k)  = (qc1d(k-1)*XZ - XD* qc1d(k-1) )/ denom 
               qi1d(k)  = (qi1d(k-1)*XZ - XD* qi1d(k-1) )/ denom 
           
               nc1d(k)  = (nc1d(k-1)*XZ - XD* nc1d(k-1) )/ denom 
               ni1d(k)  = (ni1d(k-1)*XZ - XD* ni1d(k-1) )/ denom 
     
               nwfa1d(k)= (nwfa1d(k-1)*XZ - XD* nwfa1d(k-1) + XE* nwfa1d_env(k-1) )/ denom 
               nifa1d(k)= (nifa1d(k-1)*XZ - XD* nifa1d(k-1) + XE* nifa1d_env(k-1) )/ denom 
...
!--the same for 
      ...
      !  /&
      ! (1.+((2.8e6)**2) *0.622 *qvs(k)/(1005*287*t1d(k)**2))
      !     qvs(k) = rslf( p1d(k), t1d(k))
      print*,"QCQVTK-1=",k,qc1d(k-1),qv1d(k-1),t1d(k-1),(XZ - XD )/ denom 
      print*,"QCQVTDT =",k,qc1d(k),qv1d(k),t1d(k),(g/cp)*dz1d_cup(k-1)
      !.. print*,"Xk2=",k,(1.+((2.8e6)**2) *0.622 *qvs(k)/(1005*287*t1d(k)**2)),&
                       !.. 1.+2.8e+6*qvs(k)/(287*t1d(k))
...
!-srf--------------------------------------------------------------------
   ...
   !print*,"rho=",k,rho(k),pres(k),temp(k),qv(k)
...
!srf     if (qg1d(k) .gt. R1) then
         if (qg1d(k) .gt. R1 .AND. GRAUPEL) then
            no_micro = .false.
            rg(k) = qg1d(k)*rho(k)
            L_qg(k) = .true.
         else
...
!+---+-----------------------------------------------------------------+
!     if (debug_flag) then
!      write(mp_debug,*) 'DEBUG-VERBOSE at (i) ', ii, ', ', jj
!      CALL wrf_debug(550, mp_debug)
!      do k = kts, kte
...
!srf    do k = kts, kte
   ...
   !print*,"qs=",k, qv(k),qv(k)/qvsi(k),qv(k)/qvs(k),qc1d(k)
         satw(k) = qv(k)/qvs(k)
         sati(k) = qv(k)/qvsi(k)
         ssatw(k) = satw(k) - 1.
         ssati(k) = sati(k) - 1.
...
!-srf-------------------- get vertical velocity of the updfraft-----------------------
!-srf-part_1
!print*,"NS=",k,  nc1d(k)  ,ni1d  (k),nwfa1d(k),nifa1d(k)
      PRINT*,"DEN=",k,1,(lvap(k))**2*0.622*qvs(k)/(Cp*R*temp(k)**2),&
                  lvap(k)*qvs(k)/(R*temp(k)), &
           (1.+lvap(k)*qvs(k)/(R*temp(k)))/&
           (1.+(lvap(k))**2*0.622*qvs(k)/(Cp*R*temp(k)**2))

IF(k.le.kbcon) w1dtmp(k)=2.
IF(k >kbcon-1 + nsteps .and. K<kte)then

        Tve=  (t1d_env(k)  *(1.+(qv1d_env(k)  /0.622)/(1.+qv1d_env(k  ))))! + &
!               t1d_env(k+1)*(1.+(qv1d_env(k+1)/0.622)/(1.+qv1d_env(k+1))) )*0.5
 
        Tv =  (temp(k)  *(1.+(qv(k)  /0.622)/(1.+qv (k  ))))! + &
!               temp(k+1)*(1.+(qv(k+1)/0.622)/(1.+qv (k+1))))*0.5
!print*,"ENV3=",k,qv1d(k),qv1d_env(k),temp(k),t1d_env(k)    
        
        qtot= (qc1d(k)+qi1d(k))!+qc1d(k+1)+qi1d(k+1))*0.5
        BU  =  g*( (Tv-Tve)/Tve - qtot)
        dw1 =  2./(f*(1.+gam)) * BU * dzq(k) 
 
        kx  = (1.+beta*C_d)*max(en1d(k),cd1d(k))*dzq(k)
  
        dw2 = w1dtmp(k-1)**2 - 2.*kx*w1dtmp(k-1)**2
        
  w1dtmp(k)=(dw1+dw2)/(1.+kx)
  
  if(w1dtmp(k)<0.) w1dtmp(k)=0.25*w1dtmp(k-1)
  w1dtmp(k)=sqrt(w1dtmp(k))
w1dtmp(k)=max(1.,w1dtmp(k))
  
  !.. dt = dz1d_cup(k)/w1dtmp(k)
  !.. dtsave=dt
  !.. odt=1./dt
  !.. odts=odt

print*,"W=",k,w1dtmp(k),tv,tve,dt
ENDIF
w1d(k)=w1dtmp(k)
...
!+---+-----------------------------------------------------------------+
      IF (.not. iiwarm) THEN
!srf    do k = kts, kte
!srf    if (.not. L_qs(k)) CYCLE
        if ( L_qs(k)) then !added srf
...
!+---+-----------------------------------------------------------------+
        ...
        IF(k==kts) then
           k_0 = kts
         do kk = kte, kts, -1
           if (temp(kk).ge.270.65) k_0 = MAX(k_0, kk)
         enddo
        ENDIF
!srf    do k = kte, kts, -1
...
!.. diameters correctly computed from gamma distrib of cloud droplets.
         if (rc(k).gt. 0.01e-3) then
          ...
          tau  = 3.72/(rc(k)*taud)
          prr_wau(k) = zeta/tau
!print*,"wau1=",k,prr_wau(k) ,rc(k),odts,rc(k)*odts
...
!.. print*,"qvten1",k,qvten(k) , -pri_inu(k), - pri_iha(k), - pri_ide(k)   &
                      !.. ,- prs_ide(k), - prs_sde(k), - prg_gde(k),&
          !.. qvten(k) + (-pri_inu(k) - pri_iha(k) - pri_ide(k)   &
                      !.. - prs_ide(k) - prs_sde(k) - prg_gde(k)) &
                      !.. * orho 
...
print*,"qvten1=",k,qvten(k) 

!..Cloud water tendency
print*,"qcten1", k, qcten(k) ,-prr_wau(k), - pri_wfz(k) &
                      ,- prr_rcw(k), - prs_scw(k), - prg_scw(k) &
                      ,- prg_gcw(k),qcten(k) + (-prr_wau(k) - pri_wfz(k) &
                      - prr_rcw(k) - prs_scw(k) - prg_scw(k) &
                      - prg_gcw(k)) &
                      * orho
         ...
         qcten(k) = qcten(k) + (-prr_wau(k) - pri_wfz(k) &
                      ...
                      - prg_gcw(k)) &
                      * orho
print*,"qcten1=", k, qcten(k) 

!.. print*,"qct1=",k,qcten(k),prr_wau(k)! , pri_wfz(k) ,prr_rcw(k) 
!.. print*,"qct2=",k,prs_scw(k) ,prg_scw(k) ,prg_gcw(k)
...
! print*,"X1=",k,temp(k) , tten(k),qvten(k), qcten(k)  
         otemp = 1./temp(k)
         tempc = temp(k) - 273.15
...
print*,"1st update",k
print*," TQVQS=",k,temp(k), qv(k) ,qvs(k)
...
!srf     if ((qg1d(k) + qgten(k)*DT) .gt. R1) then
         if ((qg1d(k) + qgten(k)*DT) .gt. R1 .AND. GRAUPEL) then
...
!srf   enddo
print*,"QCQRQI=",k,qc1d(k)/rho(k), qr1d(k)/rho(k) ,qi1d(k)/rho(k)
...
!srf========================================================
!      enddo !srf - end of the first loop on the vertical column
!srf========================================================

!~ IF (.not. iiwarm) then !srf added
!~ IF ( GRAUPEL) then!srf
!~ !+---+-----------------------------------------------------------------+
!~ !..Calculate y-intercept, slope values for graupel.
!~ !+---+-----------------------------------------------------------------+
...
!srf     do k = kts, kte
    ...
    print*,"CLAP1",clap,qv(k),qvs(k),1./(1. + lvt2(k)*qvs(k)),qv(k)-qvs(k)
          do n = 1, 3
             fcd = qvs(k)* EXP(lvt2(k)*clap) - qv(k) + clap
             dfcd = qvs(k)*lvt2(k)* EXP(lvt2(k)*clap) + 1.
         print*,"CLAP1",clap,fcd,dfcd,clap - fcd/dfcd
             clap = clap - fcd/dfcd
          enddo
          xrc = rc(k) + clap*rho(k)
          xnc = 0.
print*,"1stDQC",k
print*,"DQC=",k,clap,clap*odt
          if (xrc.gt. R1) then
           prw_vcd(k) = clap*odt
!+---+-----------------------------------------------------------------+ !  DROPLET NUCLEATION
           if (clap .gt. eps) then
...
print*,"?PRWVCD=",k,clap,pnc_wcd(k)
        ...
        !+---+-----------------------------------------------------------------+
          ...
          qv(k) = MAX(1.E-10, qv1d(k) + DT*qvten(k))
          temp(k) = t1d(k) + DT*tten(k)
!print*,"X2=",k,temp(k) , t1d(k) , DT,tten(k)  
...
print*,"2ndUpdate",k, t1d(k),qc1d(k)
print*,"TEND   ",k,tten(k),qvten(k),qcten(k)
print*,"TQVQCQS",k,temp(k),qv(k),rc(k)/rho(k),qvs(k)


!-srf partial update for the transport/lateral mixing
!-srf-part_2
!if(k>kbcon+ nsteps)then
           orho    =   1./rho(k)
          t1d(k)   =   temp(k)    
         qv1d(k)   =   qv(k)
         qc1d(k)   =   rc(k)*orho
         ...
         nr1d(k)   =   nr(k)*orho 
         nc1d(k)   =   nc(k)*orho 
         nwfa1d(k) =   nwfa(k)*orho
         nifa1d(k) =   nifa(k)*orho
print*,"FINALUpdate",k
print*,"TQVQCQR",k,t1d(k),qv1d(k),qc1d(k),qr1d(k)
...
!-srf========================================================
!-srf-part_3
      enddo !end of the first loop on the vertical column
!-srf========================================================      
      do k = kts,kte
      if(maxval(w1d(kts:kte))>0.) then
!        if(k==kts)write(51,*),dt, "k, t1d(k) p1d(k) , w1d(k) , qv1d(k) ,qc1d(k),qr1d(k) ,qi1d(k)  ,qs1d(k) ,qg1d(k) ni1d(k) ,nr1d(k) ,nc1d(k) ,nwfa1d(k) nifa1d(k) "
        write(51,101) k, t1d(k)-273.15,t1d_env(k)-273.15, &
            p1d(k)*0.01   ,& 
            w1d(k)        ,&
            qv1d(k)       ,&   !1
!     qv1d_env(k)   ,& 
...
!           qg1d(k)  ,&
          ...
          101 format(1x,i4,2f8.2,1x,f7.2,1x,f7.2,11(1x,e13.6))
!         write(mp_debug, '(a,i3,f8.2,1x,f7.2,1x, 11(1x,e13.6))')        &
      
 if(k>kbcon+ nsteps)then
 
        !-get the tendencies including transport and lateral mixing 
          tten(k)  =  (    t1d(k) - b_t1d(k)    )*odt 
         qvten(k)  =  (   qv1d(k) - b_qv1d(k)   )*odt 
         qcten(k)  =  (   qc1d(k) - b_qc1d(k)   )*odt 
         qiten(k)  =  (   qi1d(k) - b_qi1d(k)   )*odt 
         qrten(k)  =  (   qr1d(k) - b_qr1d(k)   )*odt 
         qsten(k)  =  (   qs1d(k) - b_qs1d(k)   )*odt 
         qgten(k)  =  (   qg1d(k) - b_qg1d(k)   )*odt 
         niten(k)  =  (   ni1d(k) - b_ni1d(k)   )*odt 
         nrten(k)  =  (   nr1d(k) - b_nr1d(k)   )*odt 
         ncten(k)  =  (   nc1d(k) - b_nc1d(k)   )*odt 
         nwfaten(k)=  ( nwfa1d(k) - b_nwfa1d(k) )*odt 
         nifaten(k)=  ( nifa1d(k) - b_nifa1d(k) )*odt 
         !- get back the inital state for consitency with the rest of the code  
          t1d(k)   =   b_t1d(k)
         qv1d(k)   =   b_qv1d(k)
         qc1d(k)   =   b_qc1d(k)
         qi1d(k)   =   b_qi1d(k)
...
! Goal: less prominent size sorting
      ...
      print*,"dtq=",k, dzq(k),delta_tp,nstep,MAX(vtrk(k),vtnrk(k))
         endif
      enddo
      ...
      do n = 1, nstep
         ...
         do k = ksed1(1), kts, -1
            ...
            nr(k) = MAX(R2, nr(k) + (sed_n(k+1)-sed_n(k)) &
                                           *odzq*DT*onstep(1))
     !print*,"xr=",k,ksed1(1),nr(k),sed_n(k),pres(k)/100.
         enddo
...
!    print*,"BUG=",nc1d(k),rho(k),nu_c,k;call flush(6)
...
!..Local variables
      ...
      IF ( wrf_dm_on_monitor ) THEN
        ...
        close(iunit_mp_th1)
        !Print*,"tnccn_act:",maxval(tnccn_act),minval(tnccn_act)
      ENDIF
...
!+---+-----------------------------------------------------------------+
END MODULE module_mp_thompson_GF
!+---+-----------------------------------------------------------------+
